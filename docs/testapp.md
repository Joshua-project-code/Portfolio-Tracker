...
| TC-070 | `test_save_seaborn_position_distribution_pie_chart_creates_file` | Generates a Seaborn-styled position distribution pie chart. | Creates `seaborn_sector_distribution_<CURRENCY>_YYYY-MM-DD.png`. |
...
| TC-073 | `test_save_plotly_position_distribution_pie_chart_creates_html_file` | Generates a Plotly position distribution pie chart. | Creates `plotly_sector_distribution_<CURRENCY>_YYYY-MM-DD.html` containing Plotly markup. |
...
| TC-076 | `test_fill_missing_stock_codes_from_mapping_uses_old_stock_names` | Uses saved current and historical stock names to fill missing position stock codes before country exposure calculation. | Missing and blank position stock codes are filled from saved current or historical stock names. |
...
| TC-078 | `test_aggregate_country_totals_for_pie_keeps_top_four_plus_others` | Aggregates country totals for pie charts with a caller-supplied slice limit. | With `max_slices=5`, keeps the top four countries and combines the remaining value into `Others`. |
| TC-079 | `test_save_country_exposure_pie_charts_uses_supplied_date` | Saves country exposure pie charts using the report date supplied by the caller. | Creates a country exposure PNG with the supplied `YYYY-MM-DD` date in the filename. |
| TC-080 | `test_save_dataframes_to_csv_accepts_supplied_date` | Saves transaction and position CSV files using the report date supplied by the caller. | Creates `transactions_2026-04-30.csv` and `positions_2026-04-30.csv`. |
| TC-081 | `test_calculate_portfolio_performance_metrics_returns_expected_simple_and_irr` | Calculates portfolio return metrics for complete single-currency data. | Returns simple return, annualized IRR, and TWR values for USD data. |
| TC-082 | `test_calculate_portfolio_performance_metrics_keeps_currencies_separate` | Avoids blending USD and SGD performance metrics. | Top-level return values remain empty and `by_currency` contains separate USD and SGD returns. |
| TC-083 | `test_calculate_portfolio_performance_metrics_uses_assumptions_for_incomplete_history` | Calculates return metrics when transaction history does not cover current cost basis. | Returns IRR and TWR using an explicitly stated assumed initial investment. |
