...
| `portfolio_tracker/performance_metrics.py` | Calculates portfolio annualized IRR, simple return, time-weighted return, and CAGR by currency from runtime transaction, position, and valuation dataframes, including documented dynamic assumptions for incomplete transaction history. Also calculates per-holding return metrics used by the web app table. |
