...
6. Maintain `data/stock_code_mapping.csv` as persisted generated code/name
   history and `data/stock_mapping.csv` as the editable sector/geography input.
...
- `docs/testapp.md`: Test case catalogue with each test's description and expected observed output. It currently tracks 95 tests.
...
Do not commit broker exports or dated generated outputs. `.gitignore` excludes
local broker folders, `Output`, Python caches, virtual environments, and
Matplotlib cache files. `data/stock_code_mapping.csv` is generated by report
runs, but it is intentionally trackable as persisted stock-code/name history.
